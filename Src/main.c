...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_UART_Transmit(&huart1, (uint8_t*) msgInit, strlen(msgInit), HAL_MAX_DELAY);
	XPT2046_init(&hspi1, XPT2046_LANDSCAPE, 320, 240);
	ILI9341_FillScreen(BLACK);
	char msgInitEnd[] = "InitEnd\n";
	HAL_UART_Transmit(&huart1, (uint8_t*) msgInitEnd, strlen(msgInitEnd), HAL_MAX_DELAY);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	    touch_t t = XPT2046_getTouch();
		      if(t.state == T_shortHoldDown || t.state == T_longHoldDown) {
		   	  ILI9341_DrawPixel(t.x, t.y, WHITE);
		  }
    /* USER CODE END WHILE */
